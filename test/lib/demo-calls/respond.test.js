...
import { withDbErrorHandling, ValidationError, NotFoundError, ConflictError } from "../../../lib/demo-calls/respond.js";
...
describe("withDbErrorHandling", () => {
  ...
  it("maps ConflictError to 409 and includes the existing lead", async () => {
    const res = mockRes();
    await withDbErrorHandling(res, async () => { throw new ConflictError("dup", { id: "abc", status: "active" }); });
    expect(res.statusCode).toBe(409);
    expect(res.body).toEqual({ error: "dup", existing_lead: { id: "abc", status: "active" } });
  });
