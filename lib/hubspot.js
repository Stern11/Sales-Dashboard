// Shared HubSpot API client used by every module adapter (api/abm, api/pipeline,
// api/sources). Handles auth, the search endpoints' 100-id batch cap, and the
...
export async function hubspotSearch(token, objectType, ids, properties) {
  ...
  for (const b of batches) {
    results.push(await hubspotSearchBatch(token, objectType, b, properties));
 ...
 * like Pipeline and the channel-attribution module (api/sources) that don't
 * have a fixed roster to search by ID. The returned array also carries `.total` (the filter's true match
...
export async function hubspotBatchAssociations(token, fromObjectType, toObjectType, fromIds) {
  ...
  for (const batch of chunk(fromIds, 100)) {
    ...
    if (fromIds.length > 100) await sleep(150);
  }
  return map;
}
