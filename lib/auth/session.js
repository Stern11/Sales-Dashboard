...
export async function verifySession(token, secret) {
  // Fail closed, explicitly, on a missing secret.
  //
  // Callers used to pass `process.env.SESSION_SECRET || ""`. That was never
  // forgeable — WebCrypto refuses a zero-length HMAC key outright
  // ("DataError: Zero-length key is not supported"), so an unset secret
  // failed closed by accident. But it failed as a thrown DataError from
  // hmacKey() below, which escapes this function's own try/catch blocks and
  // surfaces as a 500 on every /api/* request instead of a clean 401. This
  // makes the intended behavior the actual behavior.
