...
export async function fetchEngagementsForContact(token, contactId) {
  // Meetings and Notes are independent lookups on the same contact, so they
  // run together rather than one fully awaiting before the other starts —
  // this panel opens on demand and its latency is what the user waits on.
  // allSettled rather than all, because a Notes scope error must still
  // degrade to notes_available:false instead of failing the request, and
  // Promise.all would reject the pair.
  const [meetingsResult, notesResult] = await Promise.allSettled([
    fetchAssociatedObjects(token, contactId, "meetings", MEETING_PROPERTIES),
    fetchAssociatedObjects(token, contactId, "notes", NOTE_PROPERTIES),
  ]);

  if (meetingsResult.status === "rejected") throw meetingsResult.reason;
  const meetings = meetingsResult.value.map(normalizeMeeting);

  let notes = [];
  let notesAvailable = true;
  if (notesResult.status === "fulfilled") {
    notes = notesResult.value.map(normalizeNote);
  } else if (notesResult.reason instanceof HubspotScopeError) {
