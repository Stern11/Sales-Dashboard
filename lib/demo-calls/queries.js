...
export function summarize(leads) {
  ...
  return {
    ...
    call_1_done: active.filter((l) => Number(l.call_count) >= 1).length,
    call_2_done: active.filter((l) => Number(l.call_count) >= 2).length,
    call_3_done: active.filter((l) => Number(l.call_count) >= 3).length,
