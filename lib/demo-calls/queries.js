...
// Form fields (call_date especially — a `date` column rejects "" outright)
// arrive as "" rather than omitted when a rep leaves them blank; normalize
// to null before they ever reach a query.
...
const LEAD_COLUMNS = `
  ...
  pipeline_lead_id, added_to_pipeline_at, company_scale,
...
export async function listLeads() {
  ...
  const leads = await sql`
    select
      ...
      l.pipeline_lead_id, l.added_to_pipeline_at, l.company_scale,
...
export function summarize(leads) {
  ...
  return {
    ...
    call_1_done: active.filter((l) => Number(l.completed_count) >= 1).length,
    call_2_done: active.filter((l) => Number(l.completed_count) >= 2).length,
    call_3_done: active.filter((l) => Number(l.completed_count) >= 3).length,
...
export async function createLead(fields) {
  ...
  const {
    ...
    hubspot_contact_id = null, hubspot_origin_module = null, company_scale = null,
    actor, first_call = null,
  } = fields;
  ...
  const statements = [
    sql`
      insert into demo_call_leads (
        id, company_name, contact_name, email, phone,
        hubspot_contact_id, hubspot_origin_module, company_scale, created_by, updated_by
      ) values (
        ${id}, ${company_name}, ${contact_name}, ${email}, ${phone},
        ${hubspot_contact_id}, ${hubspot_origin_module}, ${blankToNull(company_scale)}, ${actor}, ${actor}
      )
...
const EDITABLE_FIELDS = ["company_name", "contact_name", "email", "phone", "company_scale"];
...
export async function addCall(leadId, fields, actor) {
  const sql = getSql();
  const countRows = await sql`select count(*)::int as n from demo_call_logs where lead_id = ${leadId}`;
  const callNumber = countRows[0].n + 1;
  const id = randomUUID();
  ...
  const [[call]] = await sql.transaction([
    sql`
      insert into demo_call_logs (
        ...
        ${id}, ${leadId}, ${callNumber}, ${blankToNull(call_date)}, ${outcome}, ${blankToNull(notes)}, ${blankToNull(next_steps)}, ${blankToNull(transcript_url)}, ${actor}, ${actor}
      )
