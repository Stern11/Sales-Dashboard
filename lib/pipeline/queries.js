...
const LEAD_COLUMNS = `
  ...
  hubspot_contact_id, hubspot_origin_module, company_scale, is_supply_chain,
...
export async function listLeads() {
  const sql = getSql();
  const leads = await sql`select ${sql.unsafe(LEAD_COLUMNS)} from pipeline_leads order by updated_at desc`;
...
export async function listNotes(leadId) {
  const sql = getSql();
  return sql`select id, lead_id, body, author, created_at from pipeline_lead_notes where lead_id = ${leadId} order by created_at desc`;
}
...
export async function createLead(fields) {
  ...
  const {
    ...
    company_scale = null, is_supply_chain = false, priority = "medium", deal_size = null,
  ...
  const [[lead]] = await sql.transaction([
    sql`
      insert into pipeline_leads (
        id, company_name, contact_name, email, phone, source, source_locked,
        hubspot_contact_id, hubspot_origin_module, company_scale, is_supply_chain, priority,
...
const EDITABLE_FIELDS = [
  "company_name", "contact_name", "email", "phone", "source", "company_scale",
...
export async function updateLead(id, fields, actor) {
  ...
  const rows = await sql`
    update pipeline_leads set
      ...
      phone = ${next.phone},
      source = ${next.source},
      company_scale = ${next.company_scale},
      is_supply_chain = ${next.is_supply_chain},
      priority = ${next.priority},
      deal_size = ${next.deal_size},
...
export async function addNote(leadId, { body, author }) {
  ...
  const [[note]] = await sql.transaction([
    sql`insert into pipeline_lead_notes (id, lead_id, body, author) values (${id}, ${leadId}, ${body}, ${author}) returning id, lead_id, body, author, created_at`,
