...
// notification, triggered from handleAddNote in api/pipeline/[id]/index.js.
// Plain `fetch` against Resend's HTTP API, no SDK dependency, mirroring
// lib/hubspot.js's style. Callers must treat send failures as best-effort
// (see that handler) — a broken/missing RESEND_API_KEY should never block a
// note from saving.
//
// Recipients are restricted to the allowed sign-in domain by the caller;
// see resolveTaggedEmails in api/pipeline/[id]/index.js.
...
/**
 * Absolute URL of a lead's detail drawer, for the "View the lead" button.
 *
 * The origin comes from configuration, never from the request. It used to be
 * built from `req.headers.host` and `x-forwarded-proto`, both of which the
 * caller controls: anyone who could trigger a tagged-note email could choose
 * the domain the button in a colleague's inbox pointed at, sent from our own
 * verified sender. APP_ORIGIN is set explicitly; VERCEL_PROJECT_PRODUCTION_URL
 * is injected by Vercel and is a hostname only, so it gets the scheme added.
 *
 * Returns null when neither is configured, and the caller omits the button
 * entirely — a notification without a link is fine; a link to wherever the
 * request claimed to come from is not.
 */
function leadUrl(leadId) {
  const configured = process.env.APP_ORIGIN
    || (process.env.VERCEL_PROJECT_PRODUCTION_URL ? `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}` : null);
  if (!configured) return null;
  return `${configured.replace(/\/+$/, "")}/pipeline?lead=${encodeURIComponent(leadId)}`;
}
...
/** Sends the "you were tagged" notification for one recipient. Throws on failure — callers decide how to handle that (handleAddNote in api/pipeline/[id]/index.js swallows it per-recipient). */
export async function notifyTagged({ to, actor, companyName, contactName, stage, priority, dealSize, leadId, noteBody }) {
        ...
        <tr>
          <td style="padding:20px 28px 24px;">
            ...
            <div style="margin-top:22px;">
              <a href="${escapeHtml(url)}" style="display:inline-block;background:${BRAND};color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:12px 22px;border-radius:8px;">View the lead →</a>
            </div>` : ""}
          </td>
        </tr>
        <tr>
